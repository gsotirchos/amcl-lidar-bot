...
class CommandRobotServer {
  public:
    CommandRobotServer() {
        ...
        // Define a drive /surveyor/command_robot service with a handle_drive_request callback function
        service_ = n_.advertiseService("/surveyor/command_robot", &CommandRobotServer::handle_drive_request, this);
    }

    // This method publishes the requested linear x and angular velocities to the robot wheel joints
    bool handle_drive_request(
        surveyor::DriveToTarget::Request& req,
        surveyor::DriveToTarget::Response& res
    ) {
